...
-  Page 02-c: $\sqrt 2$: This is about the proof $\sqrt 2 \notin \mathbb Q$. Here is the proof as you would find it in a script (or school book):   Assuming that there are $m$ and $n$ such that $\sqrt{2} = m/n$, then  $2n^2 = m^2$. Let $m$ and $n$ be relatively prime. Then $2 ∣ m^2$. Since $m^2$ is even, $m$ must also be even, so $m = 2*a$ for some $a$. Thus $2*n^2 = 4 * a^2$ or $n^2 = 2 * a^2$. This means that $n^2$ is even, and as just argued, $n$ would then be even. However, this contradicts the complementary division of $m$ and $n$. This proof is formalized here. (Note that the proof given here only works for $\sqrt 2$, but not for $\sqrt 3$. The reason is that we use that for every $m\in\mathbb N$ either $m$ or $m+1$ is even (i.e. divisible by 2). This is obviously false for $3$.)
-  Page 02-d: induction\
...
-  Leaf 02-e: Pigeonhole principle\
...
Chapter. If we want to prove the statement `P → P` (i.e. `P` follows`P`) we enter the following on the left side in `vscode`:
```lean
