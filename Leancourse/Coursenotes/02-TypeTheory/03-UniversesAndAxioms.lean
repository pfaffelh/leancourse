...
The words *beta*, *eta*, and the rest come from the *lambda calculus*
of Alonzo Church in the 1930s, where functions are written
`fun x => e` and each rewriting rule was labelled with a Greek letter
(the tradition was systematized by Curry and later extended by proof
assistants).  A *reduction* is one directed computation step, `term ⟶
simpler term`.  Lean's kernel uses six of them:
...
-- is exactly the equivalence generated by these reductions, together
with eta and proof irrelevance.  And that is *precisely* what `rfl`
...
illustration is that `Nat`'s addition is defined by recursion on its
*second* argument:
