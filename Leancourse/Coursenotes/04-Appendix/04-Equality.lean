...
Definitional equality is generated by the kernel's *reduction rules* (α, β, η, δ, ι, ζ, together with proof irrelevance and quotient computation); these, and the cases where `rfl` gets *stuck*, are introduced in full in {ref "reduction-rules"}[the section on reduction rules] of the chapter on the natural numbers.
